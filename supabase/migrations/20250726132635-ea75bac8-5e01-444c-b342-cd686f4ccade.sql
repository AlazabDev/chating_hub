-- إنشاء جداول AI Platform المطلوبة
...
CREATE TABLE IF NOT EXISTS public.code_suggestions (
  ...
  line_number INTEGER,
  suggestion_type TEXT CHECK (suggestion_type IN ('fix', 'optimization', 'feature', 'security')),
...
CREATE INDEX IF NOT EXISTS idx_code_suggestions_repository_id ON public.code_suggestions(repository_id);
...
-- سياسات الأمان لاقتراحات الكود
CREATE POLICY "Users can view code suggestions for their repositories" ON public.code_suggestions
  FOR SELECT USING (
    EXISTS (
      SELECT 1 FROM public.repositories 
      WHERE id = code_suggestions.repository_id AND manager_id = auth.uid()
    )
  );

CREATE POLICY "Users can create code suggestions for their repositories" ON public.code_suggestions
  FOR INSERT WITH CHECK (
...
) AS stage_data(name, order_num, desc, prompt, automated)
WHERE NOT EXISTS (
